...
describe('staff forms and permissions',()=>{
  ...
  it('accepts only authorized internal return paths',()=>{
    ...
    for(const path of ['/users','/users/new','/users/123']) { expect(safeReturn(path,admin)).toBe(path); expect(safeReturn(path,operator)).toBe('/orders') }
    for(const path of ['/legal-documents','/legal-documents/new','/legal-documents/audit','/legal-documents/11111111-1111-1111-1111-111111111111']) { expect(safeReturn(path,admin)).toBe(path); expect(safeReturn(path,senior)).toBe('/orders'); expect(safeReturn(path,operator)).toBe('/orders') }
    expect(safeReturn('/privacy-requests',admin)).toBe('/privacy-requests'); expect(safeReturn('/privacy-requests',senior)).toBe('/privacy-requests'); expect(safeReturn('/privacy-requests',shift)).toBe('/privacy-requests'); expect(safeReturn('/privacy-requests',operator)).toBe('/orders')
    for(const user of [admin,senior,shift,operator]) expect(safeReturn('/orders',user)).toBe('/orders')
    ...
    expect(safeReturn('/home',admin)).toBe('/users');expect(safeReturn('/home',operator)).toBe('/orders');expect(safeReturn('/profile',admin)).toBe('/users/7');expect(safeReturn('/profile',operator)).toBe('/profile')
