...
describe('staff views',()=>{
  ...
  it('renders loading/recovery, login and staff shell with independent versions and logout',async()=>{
    ...
    h.session.restoreSession.mockImplementationOnce(()=>{h.session.restoreProblem.value=null;return Promise.resolve()});await button(w,'Повторить').trigger('click');await flushPromises();expect(h.router.replace).toHaveBeenCalledWith('/users');expect(w.text()).toContain('Сервер 0.0.6');expect(w.text()).toContain(`Клиент ${version}`);expect(w.text()).toContain('Иванов Иван');expect(w.find('nav a[href="/users"] i').attributes('data-icon')).toBe('$staff');expect(w.find('nav a[href="/legal-documents"] i').attributes('data-icon')).toBe('$legalDocuments');expect(w.find('nav a[href="/customer-consents"]').exists()).toBe(false);expect(w.find('nav a[href="/users/1"] i').attributes('data-icon')).toBe('$profile');expect(w.find('nav a[href="/users/1"]').text()).toContain('Мой профиль');expect(w.findAll('nav a').slice(0,6).map(link=>link.text())).toEqual(['Заказы','Магазины','Перс. данные','Правовые документы','Пользователи','Мой профиль'])
