...
describe('official exchange-rate display', () => {
  ...
  it.each([undefined, null, {}, [], [null], [{ ...rate, baseCurrency:1 }], [{ ...rate, baseCurrency:978 }], [rate, rate],
    ...Object.entries({ provider:'other', quoteCurrency:978, nominal:0, officialRate:0, sourceEffectiveDate:'2026-02-30' }).map(([key,value]) => [{ ...rate,[key]:value }]),
    ...[null, '643'].map(quoteCurrency => [{ ...rate,quoteCurrency }]),
    ...
    expect(exchangeRateDisplay(rates, currencies)).toBeNull()
  })
  it.each([undefined, null, {}, [], [null], currencies.slice(0, 1), [...currencies, currencies[0]],
    currencies.map(currency => ({ ...currency, value:0 })),
    currencies.map(currency => ({ ...currency, name:'' })),
    currencies.map(currency => ({ ...currency, routeAlias:'usd' })),
    [{ value:643,name:'Рубль',routeAlias:'rub' },{ value:643,name:'Доллар',routeAlias:'usd' }]
  ])('rejects a missing or malformed currency catalogue (%j)', catalogue => {
    expect(exchangeRateDisplay([rate], catalogue)).toBeNull()
  })
  it('renders an accessible unavailable placeholder and updates reactively', async () => {
    ...
    await wrapper.setProps({ rates:[rate], currencies })
