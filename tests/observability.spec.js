...
describe('UI observability', () => {
  it('preserves application defaults for explicitly undefined options', async () => {
    vi.resetModules()
    vi.stubGlobal('__SARAFAN_BACKOFFICE_RUNTIME_CONFIG__', { loggingEnabled: true })
    try {
  ...
  it('keeps only typed allowlisted attributes and valid W3C correlation', () => {
    const attributes = sanitizeAttributes(EVENTS.apiRequestFailed, {
      ...
      'error.type': 'https://sarafan.sw.consulting/problems/invalid-backoffice-refresh-token',
      'sarafan.problem.code': 'invalid_backoffice_refresh_token',
    ...
    expect(sanitizeAttributes(EVENTS.apiRequestFailed, {
      ...
      'http.route': '/api/v1/backoffice/users/me?token=secret',
  ...
  it('emits frozen human-readable records without retaining rejected fields', () => {
    ...
    expect(records[0]).toMatchObject({
      ...
      severityText: 'WARN',
      severityNumber: 13,
      eventName: 'sarafan.back.office.api.request.failed',
    ...
    expect(formatConsoleRecord(records[0])).toBe(
      `2026-09-03T10:20:30.456Z WARN sarafan.back.office.api.request.failed trace_id=${TRACE_ID} span_id=${SPAN_ID} API request failed: POST /api/v1/auth/refresh -> 401.`
    )
  })
  ...
  it('routes readable lines to console methods by severity', () => {
    ...
    for (const [severityNumber, method] of [[21, 'error'], [13, 'warn'], [9, 'info'], [5, 'debug']]) {
      sink.emit({
        ...
        severityText: 'TEST',
        severityNumber,
        eventName: 'sarafan.back.office.test',
  ...
  it('reports only safe problem identity and never localized or native messages', () => {
    ...
    const problem = new ProblemError({
      type: 'https://sarafan.sw.consulting/problems/invalid-backoffice-access-token',
      code: 'invalid_backoffice_access_token',
