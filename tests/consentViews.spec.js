...
it('requires a current server preview before immutable creation and preserves failed forms', async () => {
  ...
  expect(groups[0].findAll('button').map(button => button.attributes('aria-label'))).toEqual(['Предварительный просмотр'])
  expect(groups[1].findAll('button').map(button => button.attributes('aria-label'))).toEqual(['Обновить данные', 'Сохранить документ', 'Вернуться к списку'])
  expect(wrapper.get('button[aria-label="Сохранить документ"]').attributes('form')).toBe('legal-document-form')
  await click('Предварительный просмотр')
