...
export function validateStore(value, ops, id) {
  if (!value || !positive(value.id) || (id !== undefined && value.id !== Number(id))
    ...
    || !(value.logoUrl === null || (typeof value.logoUrl === 'string' && new RegExp(`^/api/v1/backoffice/stores/${value.id}/logo\\?v=[0-9a-f]{64}$`, 'u').test(value.logoUrl)))
    || (value.status === 1 && value.logoUrl === null)) protocol()
  return value
}
