...
import { validateOrderPricing, validatePricingOps } from '../orderPricing.js'
...
async function load() {
  ...
  try {
    ...
    if (current === version) pricing.value = validateOrderPricing(calculation, pricingOps.value, number.value)
  } catch (value) {
...
function save(...args) {
  ...
  return focusAfter(() => saveAction(...args), () => validationFields(fieldProblem.value))
}
