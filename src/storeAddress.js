...
export function normalizeStoreAddress(value, productSourceUrlOps = null) {
  if (typeof value !== 'string') return null
  const trimmed = value.trim()
  const maximumLength = productSourceUrlOps?.maximumLength ?? MAXIMUM_URL_LENGTH
  if (value.includes('\\') || [...value].some(char => char.codePointAt(0) < 32 || char.codePointAt(0) === 127)) return null
