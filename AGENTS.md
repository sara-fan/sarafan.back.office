...
- `/orders` is a read-only server table available through `manualQuotes` to Administrator, Shift manager, Senior operator and Operator. Administrator still lands on `/users`; the other three roles land on `/orders`. Keep Orders in the shared drawer and safe-return allowlist, with no row action, detail route or editor.
