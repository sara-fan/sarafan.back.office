...
- Product review amendment v1.22 and back.office #15 govern the flat order editor. Only server canEditProduct permits store and product changes; buyer, source and the remaining recognition fields stay read-only. Validate full details and productLimits DTOs, use staff-only GET/PUT, preserve expectedUpdatedAt verbatim, and use Core valueLimit.exceededMessage for the reserve-aware error. Compare USD integer cents against Core maximumTotalUsd; never duplicate k in the client. Follow Logibooks GtcParcel_EditDialog with a two-column flat form, horizontal labels and a full-width product-name row; reuse the back-office user editor's label and input styling.
