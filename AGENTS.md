...
- Product review amendment v1.22 and back.office #15 govern the flat order editor. Only server canEditProduct permits product changes; buyer, source and recognition stay read-only. Validate full details and productLimits DTOs, use staff-only GET/PUT, preserve expectedUpdatedAt verbatim, and use Core valueLimit.exceededMessage for the reserve-aware error. Compare USD integer cents against Core maximumTotalUsd; never duplicate k in the client.
